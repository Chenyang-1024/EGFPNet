...
def parse_args():
    ...
    parser.add_argument('--name', default='EGFPNet_coarse',
    ...
    parser.add_argument('--loss', default='BCEDiceLos  s')
...
def train(args, model, criterion, criterion_edge, optimizer, image_people):
    losses = AverageMeter()
    ious = AverageMeter()
    ...
    for j in range(int(len(image_people) // args.batch_size)):
        for k in range(args.batch_size):
            train_image[k, :, :, :] = image_people[j * args.batch_size + k][0]
            train_label[k, :, :, :] = image_people[j * args.batch_size + k][1]
            edge_label_nd[k, :, :, :] = image_people[j * args.batch_size + k][2]
...
def main_label(args, groups, label):
    ...
    for repeats in range(five):
        ...
        for epoch in range(args.epochs):
            ...
            val_log = validate(args, model, criterion, validate_image_people)
            ...
            if val_log['iou'] > best_iou:
                ...
                test = validate(args, model, criterion, test_image_people)
                trigger = 0
            ...
            if not args.early_stop is None:
                if trigger >= args.early_stop:
                    ...
                    print('val_loss %.4f - val_iou %.4f' % (test['loss'], test['iou']))
                    break
...
if __name__ == '__main__':
    ...
    datasets_train = np.load(
        '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/methods_data/edge_pyramid_net/datasets/acdc/label1/acdc_dataset_label1_coarse_groups_1.npz',
        allow_pickle=True)
    groups_1 = datasets_train['people']

    #groups 2
    datasets_validate = np.load(
        '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/methods_data/edge_pyramid_net/datasets/acdc/label1/acdc_dataset_label1_coarse_groups_2.npz',
        allow_pickle=True)
    groups_2 = datasets_validate['people']

    #groups 3
    datasets_test = np.load(
        '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/methods_data/edge_pyramid_net/datasets/acdc/label1/acdc_dataset_label1_coarse_groups_3.npz',
        allow_pickle=True)
    groups_3 = datasets_test['people']

    #groups 4
    datasets_train = np.load(
        '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/methods_data/edge_pyramid_net/datasets/acdc/label1/acdc_dataset_label1_coarse_groups_4.npz',
        allow_pickle=True)
    groups_4 = datasets_train['people']

    #groups 5
    datasets_validate = np.load(
        '/media/csl/5b58b9b5-0162-4611-b882-6351023584d1/csl/methods_data/edge_pyramid_net/datasets/acdc/label1/acdc_dataset_label1_coarse_groups_5.npz',
        allow_pickle=True)
